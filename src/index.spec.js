...
describe('HTML Thingy', () => {
	...
	it('should render lists', () => {
		const Item = html`<li>${(props) => props.title}</li>`;
		const items = [ 'foo', 'bar' ];
		const mapItems = () => items.map((title) => Item({ title }));
		const Test = html`<ul>${mapItems}</ul>`;

		render(Test(), null, '#app');
		expect(document.querySelector('#app').innerHTML).eql('<ul><li>foo</li><li>bar</li></ul>');
	});
	/*
	it('should rerender template list state', () => {
		const state = observable({
			list: []
		});
		const Test = html`<h1>hello ${(_, state) => state.foo}</h1>`;
