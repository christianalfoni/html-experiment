	...
	_renderPart(node) {
		...
		autorun(() => {
			if (this.element) {
				...
				if (Array.isArray(partContent)) {
					this.type = 'list';
					this.element = document.createDocumentFragment();
					partContent.forEach((template) => {
						this.element.appendChild(template.get(this.template.props, this.template.state).childNodes[0]);
					});
				} else if (partContent instanceof Template) {
					this.type = 'template';
					this.element = partContent.get(this.template.props, this.template.state).childNodes[0];
				} else {
					this.type = 'text';
			...
			if (node.nodeType === 8 && this._hasMarkerId(node.textContent)) {
				...
				for (let x = 0; x < attributes.length; x++) {
					...
					if (this._hasMarkerId(attribute.textContent)) {
						...
						const part = this.parts[this.partsIndexById[id]];
						this.partsById[id] =
							attribute.name.substr(0, 3) === 'on-'
								? new EventPart(node, attribute.name.substr(3), part, this)
								: new AttributePart(node, attribute.name, part, this);
					}
				}
			}
		}
